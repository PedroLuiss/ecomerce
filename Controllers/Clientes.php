...
class Clientes extends Controller
{
    ...
    public function registrar()
    {
        if (isset($_POST['nombre']) && isset($_POST['apellido']) && isset($_POST['direccion'])) {
            ...
            if (
                empty($nombre) || empty($apellido) || empty($direccion) || empty($estado_id)|| empty($ciudade_id)|| empty($municipio_id)|| empty($parroquia_id)
            ) {
                $respuesta = array('msg' => 'Todo los campos son requeridos', 'type' => 'warning');
            } else {
                if (empty($id)) {
                    $data = $this->model->registrar($correo, $nombre, $apellido, $direccion, $tipo,$estado_id,$ciudade_id,$municipio_id,$parroquia_id);
                    if ($data > 0) {
                    ...
                    $data = $this->model->modificar($correo, $nombre, $apellido, $direccion,$estado_id,$ciudade_id,$municipio_id,$parroquia_id, $id);
                    if ($data == 1) {
